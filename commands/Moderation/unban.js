...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!((args[0].match(/\d+/g) && args[0].length == '848658543179202611'.length) || args[0].match(/<@!?\d+>/g))) return message.channel.send(deniedEmbed('Invalid. Try pinging the user or providing in their ID.')).then(x => {x.delete({timeout:5000})})
    let mention = (args[0].match(/<@!?\d+>/g) || args[0].match(/\d+/g))
    let unbanID;
    ...
    let returnmaybe = 0
    await message.guild.members.unban(unbanID, banreason).catch(e => {
      returnmaybe = 1
    ...
    console.log(returnmaybe)
    if (returnmaybe == 0) message.channel.send(banembed).then(x => {x.delete({timeout:15000})})
    if (returnmaybe == 0) try {unbannedUser.send(banembed)} catch (err) {message.channel.send('The user could not receive any details about this incident in DMs.')}
  }
}